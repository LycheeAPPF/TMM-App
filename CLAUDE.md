...
Unofficial Android app (`io.github.lycheeappf.tmm`) that bridges phone messenger notifications to a Tesla over Bluetooth MAP/PBAP and adds an in-car **xAI Grok** voice assistant. Two core features: (1) a **message bridge** — captures messenger notifications and injects them as fake inbound SMS so the car's MAP UI reads them aloud and routes the driver's dictated reply back to the original app; (2) a **Grok assistant** — the driver dictates to a fake "Grok" contact, the reply is sent to the xAI Responses API and the answer is injected back as a fake inbound SMS. Single Gradle module (`:app`), Android 13+ (minSdk 33), Kotlin/Compose/Hilt. **All user-facing UI is German**; code/comments are mixed German+English. Developed on **Windows**.
...
| `core/` | DI modules (`di/`), security (Keystore), util (Clock, LogBuffer, SendBudget), model, network |
...
**Grok assistant** (`channel/llm/`): fake "Grok" contact (`+88810000000`, reserved mapping 0, never expires). Driver dictates → `LlmChannel.handleTeslaReply` → `LlmTurnRunner.run` (single atomic turn under per-session Mutex: TTL check → rate-limit → `GrokProvider.complete` POST `/v1/responses` with `store=false` and full in-memory history → `LlmResponseFormatter` strips Markdown, caps 800 chars) → answer injected back as inbound SMS via `maybeInjectFollowUp`. Context is in-memory only (`LlmConversationStore`, ~120s inactivity TTL), wiped on restart. Gated on privacy consent + Keystore API key — **re-checked at turn time** because the Tesla auto-reply path bypasses `LlmStarter` and the static mapping never expires. Optional user-named **voice alias** (`+88810000001`, no DB row) — `OutboundSmsClassifier` must redirect it to canonical Grok id 0 *before* DB/parse lookups.
