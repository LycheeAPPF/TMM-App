...
class InjectedMessageLedgerTest {
    ...
    @Test fun `mark then shouldIgnore returns true for same address and body`() {
        ledger.markInjected("+88810000001", "Hallo Welt")
        assertThat(ledger.shouldIgnoreOutbound("+88810000001", "Hallo Welt")).isTrue()
    }

    @Test fun `shouldIgnore is false for unknown body`() {
        ledger.markInjected("+88810000001", "Hallo Welt")
        assertThat(ledger.shouldIgnoreOutbound("+88810000001", "anderer Text")).isFalse()
    }
    ...
    @Test fun `entries expire after 10 seconds`() {
        ledger.markInjected("+88810000001", "Hallo")
        nowMs += 10_001
        assertThat(ledger.shouldIgnoreOutbound("+88810000001", "Hallo")).isFalse()
    }

    @Test fun `entries within 10 seconds still match`() {
        ledger.markInjected("+88810000001", "Hallo")
        nowMs += 9_500
        assertThat(ledger.shouldIgnoreOutbound("+88810000001", "Hallo")).isTrue()
    }
    ...
    @Test fun `mark with pure number matches outbound with bracket display+number`() {
        ...
        // beide Wege strippen.
        ledger.markInjected("+88810000007", "Welcome")
        assertThat(ledger.shouldIgnoreOutbound("Grok <+88810000007>", "Welcome")).isTrue()
    }

    @Test fun `mark with bracket form matches outbound with pure number`() {
        ledger.markInjected("Grok <+88810000007>", "Welcome")
        assertThat(ledger.shouldIgnoreOutbound("+88810000007", "Welcome")).isTrue()
    }
